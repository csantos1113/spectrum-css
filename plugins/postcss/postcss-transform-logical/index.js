		...
		prepare() {
			return {
				...
				Rule(rule, { result, Declaration }) {
					const transforms = rule.nodes
						.filter((n) => n.type === "decl" && n.prop === "transform")
						.map((n) => n.value);
					if (transforms.length === 0) return;

					// Skip rules that already have a direction specified
					if (
						rule.selector.includes('[dir="ltr"]') ||
					...
					rule.walkDecls("transform", (decl) => {
						...
						values.walk((val, idx) => {
							...
							if (val.value === "matrix") {
								decl.error(
									"logical flips cannot be performed on transforms that use matrix()",
									{
										plugin: this.postcssPlugin,
									}
								);
