...
window.addEventListener("DOMContentLoaded", async () => {
	...
	[...document.querySelectorAll(".spectrum-Popover")]
		.filter(
			(popover) =>
				!popover.classList.contains("site-Search-popover") &&
				!popover.classList.contains("spectrum-Picker-popover")
		)
		.forEach((el) => new Popover(el, {}));
	document
		.querySelectorAll(".spectrum-Rating")
		.forEach((el) => new Rating(el, {}));
	[...document.querySelectorAll(".spectrum-Picker")]
		.filter((picker) => !picker.id?.startsWith("switcher"))
		.forEach((el) => new Picker(el, {}));
	[...document.querySelectorAll(".spectrum-Menu")]
		.filter((menu) => !menu.closest(".spectrum-Popover"))
